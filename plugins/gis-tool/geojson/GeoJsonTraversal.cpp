...
LonLatCoordinate ParseCoordinate(const json& node) {
    ...
    if (!IsValidWgs84Coordinate(lat, lon)) {
...
void AddCoordinateIfValid(const json& node, std::vector<LonLatCoordinate>& output) {
    output.push_back(ParseCoordinate(node));
}

std::vector<LonLatCoordinate> ParseLinearRing(const json& ringNode) {
    if (!ringNode.is_array()) {
        throw std::invalid_argument("Polygon ring must be an array of coordinates.");
    }

    std::vector<LonLatCoordinate> ring;
    ...
    const LonLatCoordinate& first = ring.front();
    const LonLatCoordinate& last = ring.back();
...
PolygonRingsWgs84 ParsePolygon(const json& polygonCoordinatesNode) {
...
void CollectCoordinatesAtDepth(const json& node, int depth, std::vector<LonLatCoordinate>& output) {
    if (depth == 0) {
        AddCoordinateIfValid(node, output);
        return;
    }
...
void CollectFromGeometry(const json& geometry, std::vector<LonLatCoordinate>& output);
void CollectPolygonsFromGeometry(const json& geometry, std::vector<PolygonRingsWgs84>& output);

void CollectFromFeature(const json& feature, std::vector<LonLatCoordinate>& output) {
...
void CollectPolygonsFromFeature(const json& feature, std::vector<PolygonRingsWgs84>& output) {
    if (!feature.contains("geometry")) {
        throw std::invalid_argument("Feature must contain geometry.");
    }
...
void CollectFromFeatureCollection(const json& collection, std::vector<LonLatCoordinate>& output) {
...
void CollectPolygonsFromFeatureCollection(const json& collection, std::vector<PolygonRingsWgs84>& output) {
    if (!collection.contains("features") || !collection["features"].is_array()) {
        throw std::invalid_argument("FeatureCollection must contain a features array.");
    }
...
void CollectFromGeometryCollection(const json& geometryCollection, std::vector<LonLatCoordinate>& output) {
...
void CollectPolygonsFromGeometryCollection(const json& geometryCollection, std::vector<PolygonRingsWgs84>& output) {
    if (!geometryCollection.contains("geometries") || !geometryCollection["geometries"].is_array()) {
        throw std::invalid_argument("GeometryCollection must contain a geometries array.");
    }
...
std::vector<LonLatCoordinate> CollectWgs84Coordinates(const json& geojson) {
    ...
    std::vector<LonLatCoordinate> coordinates;
...
std::vector<PolygonRingsWgs84> CollectPolygonsWgs84(const json& geojson) {
    ...
    const std::string type = geojson["type"].get<std::string>();
    std::vector<PolygonRingsWgs84> polygons;
